...
public class IdentityService {
  ...
  public IdentityResponse authenticate(IdentityRequest request, HttpServletRequest httpRequest,
  ...
  public byte[] renewToken(byte[] tokenBytes, long tokenId) throws IOException {
    ...
    IdentityToken newToken = new IdentityToken(identityToken.getId(), SecretsManager.get()
        .getSecrets(identityToken.getUser()), identityToken.getIssuer(), identityToken.getUser(),
        identityToken.getIssueInstant(), identityToken.getNotBefore(), newExpiryTime,
        identityToken.isEncrypted());
  ...
  /**
   * Only Authorization server can get Secrets. Identity token of authorization server is signed and
   * encrypted using Secrets of identity server. Protocol indicates Hadoop service, and it equals
   * the user principal of Hadoop service after login.
   */
  ...
  private String getAuthorizationServerPrincipal() throws IOException {
    String principal = conf
        .get(HASConfiguration.HADOOP_SECURITY_TOKENAUTH_AUTHORIZATION_SERVER_PRINCIPAL_KEY);
    if (principal == null) {
   ...
   */
  protected IdentityResponse generateResponse(String sessionId, Login login, boolean needSecrets)
      throws IOException {
    /**
     * If there are more than one principal, we only use the first one to create identity token.
     */
    ...
    return new IdentityResponse(sessionId, Login.SUCCEED, login.getFailure(),
        login.getRequiredCallbacks(), TokenUtils.getBytesOfToken(identityToken), secretKey,
        publicKey);
  }

  /**
   * Generate Identity token after login successfully. Identity token of authorization server is
   * signed/encrypted using Secrets of Identity server. Identity token of normal user or Hadoop
   * service is signed/encrypted using Secrets of Authorization server. By default, notBefore
   * timestamp is 5 minutes, and notOnOrAfter timestamp is one day.
   */
  protected Token generateIdentityToken(String user, List<Attribute> attributes) throws IOException {
    ...
    boolean tokenEncrypted = conf.getBoolean(
        HASConfiguration.HADOOP_SECURITY_TOKENAUTH_IDENTITY_TOKEN_ENCRYPTED_KEY, false);

    Token identityToken = new IdentityToken(secrets, "www.apache.org", user, instant, instant
        - fiveMins, instant + expires, tokenEncrypted);
  ...
  public boolean isAlive() {
    return httpServer != null && httpServer.isAlive() && rpcServer != null && rpcServer.isAlive();
  }

  private int doRun(String[] args) throws HadoopIllegalArgumentException, IOException,
  ...
  private void initIdentityServer() throws HadoopIllegalArgumentException, IOException {
  ...
  public static void main(String args[]) throws Exception {
    ...
    GenericOptionsParser parser = new GenericOptionsParser(new HASConfiguration(), args);
    IdentityService identityService = IdentityService.create(parser.getConfiguration());
