...
public class TokenAuthAuthenticator extends AbstractAuthenticator {
  ...
  /**
   * HTTP header prefix used by the TokenAuth client/server endpoints during an authentication sequence.
   */
  ...
  @Override
  public void authenticate(URL url, Token token) throws IOException,
  ...
  /**
   * If the specified URL does not support tokenauth authentication, a fallback {@link Authenticator} will be used.
   * <p/>
  ...
  private void doTokenAuthSequence(AuthenticatedURL.Token token) 
    ...
    try {
      ...
      if (subject == null) {
        ...
        LoginContext login = new LoginContext("", subject,
            new DefaultHASClientCallbackHandler(hasClient), new TokenAuthConfiguration());
        login.login();
      }
      
      org.apache.hadoop.security.tokenauth.token.Token 
      identityToken = subject.getPrivateCredentials(
