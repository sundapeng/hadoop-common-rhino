...
public class IdentityToken extends AbstractToken {
  public static final String NAME = "IDENTITY-TOKEN";

  public IdentityToken(Secrets secrets,
  ...
  public IdentityToken(Secrets secrets, String issuer, 
      String user, long issueInstant, long notBefore,
      long notOnOrAfter, boolean encrypted) {
    super(secrets, issuer, user, 
        issueInstant, notBefore, notOnOrAfter, encrypted);
  }
  
  public IdentityToken(byte[] rawBytes) {
    super(rawBytes);
  }
  
  public IdentityToken(byte[] rawBytes, Secrets secrets) {
    super(rawBytes, secrets);
  }
  
  public IdentityToken(long id, Secrets secrets, String issuer, String user, long issueInstant,
  ...
  void readHeaderFields(DataInput in) throws IOException {
    ...
    if(!NAME.equals(name)) {
      throw new InvalidToken("It's not a identity token.");
    }
