...
import org.apache.hadoop.fs.http.client.HttpFSTokenAuthAuthenticator;
import org.apache.hadoop.fs.http.client.HttpFSTokenAuthAuthenticator.DelegationTokenOperation;
 ...
 * {@link TokenAuthAuthenticationHandler}
 */
@InterfaceAudience.Private
public class HttpFSTokenAuthAuthenticationHandler
  extends TokenAuthAuthenticationHandler {
  ...
  static final Set<String> DELEGATION_TOKEN_OPS =
    ...
    public static final String TYPE = "tokenauth-dt";

    /**
     * Returns authentication type of the handler.
     *
     * @return <code>delegationtoken-tokenauth</code>
     */
    @Override
    ...
    public boolean managementOperation(AuthenticationToken token,
      ...
      if (DELEGATION_TOKEN_OPS.contains(op) &&
        ...
        if (dtOp.getHttpMethod().equals(request.getMethod())) {
          if (dtOp.requiresTokenAuthCredentials() && token == null) {
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
              MessageFormat.format(
                "Operation [{0}] requires TokenAuth authentication established",
                dtOp));
            ...
            try {
              ...
              switch (dtOp) {
                case GETDELEGATIONTOKEN:
                  String renewerParam =
                    request.getParameter(HttpFSTokenAuthAuthenticator.RENEWER_PARAM);
                ...
                case CANCELDELEGATIONTOKEN:
                  String tokenParam =
                    request.getParameter(HttpFSTokenAuthAuthenticator.TOKEN_PARAM);
                  if (tokenParam == null) {
                    response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                      MessageFormat.format(
                        "Operation [{0}] requires the parameter [{1}]",
                        dtOp, HttpFSTokenAuthAuthenticator.TOKEN_PARAM));
                    requestContinues = false;
                  } else {
    ...
    private static Map delegationTokenToJSON(Token token) throws IOException {
      ...
      json.put(HttpFSTokenAuthAuthenticator.DELEGATION_TOKEN_URL_STRING_JSON,
               token.encodeToUrlString());
      Map response = new LinkedHashMap();
      response.put(HttpFSTokenAuthAuthenticator.DELEGATION_TOKEN_JSON, json);
      return response;
    }
     ...
     * authentication to the {@link TokenAuthAuthenticationHandler} unless it is
     * disabled.
     *
    ...
    public AuthenticationToken authenticate(HttpServletRequest request,
      ...
      String delegationParam =
        request.getParameter(HttpFSTokenAuthAuthenticator.DELEGATION_PARAM);
